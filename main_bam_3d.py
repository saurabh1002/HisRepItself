from utils import BAM_motion_3d as datasets
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d) in enumerate(data_loader):
        ...
        p3d_out = p3d_out_all[:, seq_in:].reshape([batch_size, out_n, 17, 3])

        p3d_out_all = p3d_out_all[:, :, 0].reshape([batch_size, seq_in + out_n, 17, 3])
